...
class InstagramBot():
    ...
    def scrape_followers(self, user: User):
        # try to click on followers button
        try:
    ...
    def scrape_base(self, username: str):
        """
        Scrape base info about user (media, followers, followings count, private or not)
        ...
        user.media_count = self.scrape_count(upper_labels[0])
        user.follower_count = self.scrape_count(upper_labels[1])
        user.following_count = self.scrape_count(upper_labels[2])
    ...
    @staticmethod
    def scrape_count(label):
        """
        Scrape info about media/followers/followings count from label
        Then replace 'тыс.' and 'млн.' with appropriate numbers
        :param label: label on page which contains required info
        :return:
        """
        value = label.text.split()[0] if len(label.text.split()) == 2 else ''.join(label.text.split()[:2])
        if 'тыс.' in value:
            value = str(int(float(value.replace('тыс.', '').replace(',', r'.')) * 1000))
        elif 'млн' in value:
            value = str(int(float(value.replace('млн', '').replace(',', r'.')) * 1000000))
        return value
...
if __name__ == '__main__':
    ...
    while True:
        if exception_counter > 2:
            if len(creds) == 0:
                raise Exception('All users probably banned :(')
            bot.browser.quit()
            creds.pop(current_index)
            index = random.randint(0, len(creds)-1)
            current_index = index
            print('Now use', index, 'creds')
            bot = InstagramBot(creds[index][0], creds[index][1])
            bot.signIn()
